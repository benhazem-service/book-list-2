...
if (targetIndex !== -1) {
    ...
    let endOfBlock = targetIndex;
    if (lines[targetIndex + 1].includes('}')) {
        if (lines[targetIndex + 2].includes('});')) {
            endOfBlock = targetIndex + 2;
        } else {
            endOfBlock = targetIndex + 1;
        }
    } else if (lines[targetIndex + 1].includes('});')) {
        endOfBlock = targetIndex + 1;
    }
    
    // Remove the lines from targetIndex + 1 to endOfBlock
    lines.splice(targetIndex + 1, endOfBlock - targetIndex);
    ...
    // Because I split by \n, I will insert them array-wise
    lines.splice(targetIndex + 1, 0, ...fix);
    
    fs.writeFileSync('main.js', lines.join('\\n'));
    console.log('Fixed successfully by array splice');
} else {
    console.log('Could not find replace line');
}
