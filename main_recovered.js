...
const appDataDocRef = db.collection('appConfig').doc('data'); // Using a single document for all app data
    ...
    async function exportArchivedChosenBooksPDF(operationId) {
      try {
        ...
        const snapshot = p.chosenBooks;
        let allBooks = [];

        // أولاً بحسب ترتيب المستويات الحالي
        levels.forEach(level => {
          const levelName = level.name;
          if (snapshot[levelName]) {
            const books = snapshot[levelName];
        ...
        // ثم المستويات الموجودة في اللقطة القديمة وغير الموجودة حالياً
        const remainingLevels = Object.keys(snapshot)
          .filter(lvl => !levels.some(L => L.name === lvl))
          .sort((a, b) => a.localeCompare(b, 'ar'));
        remainingLevels.forEach(levelName => {
          const books = snapshot[levelName];
          Object.keys(books).sort((a, b) => a.localeCompare(b, 'ar')).forEach(book => {
            allBooks.push({ level: levelName, book: book, count: books[book] });
          });
        ...
        if (allBooks.length === 0) {
          showTemporaryAlert('لا تحتوي هذه اللقطة على كتب لتصديرها', 'error');
          return;
        }
        ...
        function booksTable(books) {
          let html = `<table class="books-table"><tr><th>الكتاب</th><th>المستوى</th><th>العدد</th></tr>`;
          let lastLevel = null;
          books.forEach(row => {
        ...
        const ownerName = (currentUser && (currentUser.name || currentUser.displayName || currentUser.email)) || '';
        const win = window.open('', '', 'width=900,height=700');
        win.document.write(`
          <html>
            <head>
              <title>تصدير الكتب المختارة (من الأرشيف)</title>
              <style>
                @media print {
                ...
                .pdf-header { text-align: center; font-weight: 700; font-size: 1.1em; margin-bottom: 8px; }
              </style>
            </head>
            <body>
              ${ownerName ? '<div class="pdf-header">' + ownerName + '</div>' : ''}
